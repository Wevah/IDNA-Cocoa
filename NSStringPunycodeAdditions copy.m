...
- (NSString *)punycodeEncodedString {
	...
	for (j = 0;  j < input_length;  ++j) {
		if (basic([self characterAtIndex:j])) {
			[ret appendFormat:@"%C",
				case_flags ? (unichar)encode_basic([self characterAtIndex:j], case_flags[j]) : [self characterAtIndex:j]];
			++outLen;
		}
	...
	while (h < input_length) {
		...
		for (m = maxint, j = 0;  j < input_length;  ++j) {
			...
			unsigned c = [self characterAtIndex:j];
			
			if (c >= n && c < m)
				m = [self characterAtIndex:j];
		}
...
- (NSString *)punycodeDecodedString {
	...
	for (j = 0;  j < b;  ++j) {
		...
		if (case_flags)
			case_flags[outLen] = flagged([self characterAtIndex:j]);
		if (!basic([self characterAtIndex:j]))
			return nil; //punycode_bad_input;
		[ret appendFormat:@"%C", [self characterAtIndex:j]];
		++outLen;
	}
	...
	for (inPos = b > 0 ? b + 1 : 0; inPos < input_length; ++outLen) {
		...
		[ret insertString:[NSString stringWithFormat:@"%C", n] atIndex:i++];
	}
	...
	while (![s isAtEnd]) {
		if ([s scanUpToCharactersFromSet:dotAt intoString:&input]) {
			if ([[input lowercaseString] hasPrefix:@"xn--"])
				[ret appendString:[[input substringFromIndex:4] punycodeDecodedString]];
			else
				[ret appendString:input];
		}
